...
public class TestEventReciver : Receiver
{
	...
	public override void OnNotify(Playable origin, INotification notification, object context)
	{
		...
		TestEventMarker marker = notification as TestEventMarker;
		if (marker != null)
		{
            for (int i = 0; i < marker.events.Length; i++)
            {
				...
				if (info != null)
				{
                	...
                	info.Invoke(marker.events[i].obj.GetComponent(marker.events[i].component), temp);
				}
            }
		}
	}
